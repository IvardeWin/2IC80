...
if __name__ == '__main__':
    ...
    while True:
        user_input = input(
            "Name of interface to discover on, or 'done' once finished:\n")
    ...
    if user_input == "y":
        while True:
            user_input = input(
                "Name of interface to actively discover on, or 'done' once finished:\n")
            if user_input in chosen_interfaces:
                if user_input not in active_discover_interfaces:
                    ...
                    print(
                        f"Currently selected interfaces: {active_discover_interfaces}")
                else:
                    print("This interface has already been selected")
            elif user_input == "all":
    ...
    print(
        f"Now starting discovery...  press [ENTER] once the desired hosts have been discovered.")
    for interface in chosen_interfaces:
        hosts[interface] = dict()
        discover_threads[interface] = discover_passive.Discover(
            hosts, interface)
    ...
    print("Please type the name of the interface on which the hosts you want to ARP spoof were found")
    while True:
        user_input = input()
        if user_input in hosts:
            ...
            print(
                "This is not an interface on which hosts were discovered, please type another name.")
    ...
    while True:
        ...
        while True:
            ...
            if user_input in hosts[arp_spoof_victim_if]:
                ...
                print(
                    "This is not a MAC address that was found, please type another MAC address.")
        ...
        if len(hosts[arp_spoof_victim_if][arp_spoof_victim_mac]) == 1:
            ...
            while True:
                ...
                if user_input in hosts[arp_spoof_victim_if][arp_spoof_victim_mac]:
                    ...
                    print(
                        "This is not an IP address that was found, please type another IP address.")
        ...
        if user_input == "y":
            ...
            print(
                "You should spoof at least two hosts in order to perform a man in the middle attack")
        else:
            print("Host that will be spoofed:")
            for i in range(len(arp_spoof_victim_ips)):
                print(
                    f"{arp_spoof_victim_macs[i]}  -  {arp_spoof_victim_ips[i]}")
            break

    print("Do you also want to apply DNS spoofing?")
    user_input = input("[y/n]\n")
    if user_input != "y":

        input("Setup ready, press [ENTER] to start ARP")
        arp_spoofing = list()
        for i in range(len(arp_spoof_victim_ips)):
        ...
        while True:
            ...
            if user_input in spoofed_domain_names:
                print(
                    "This domain name is already in the list of domains that will be spoofed.")
            elif user_input == "done":
                break
            else:
                spoofed_domain_names.append(user_input)
                print(
                    f"domain names that will be spoofed: {spoofed_domain_names}")

        while True:
            print(
                "Please type the IP to which the spoofed domain names should be redirected:")
            user_input = input()
            redirect_ip = user_input
            print(
                f"Are you sure you want to redirect the spoofed host to {redirect_ip}?")
            user_input = input("[y/n]\n")
